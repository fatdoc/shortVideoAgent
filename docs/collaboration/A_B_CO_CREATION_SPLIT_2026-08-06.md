...
2. 邀请链接有效期、使用次数、能否改绑代理商及归因保护期。
3. 充值支付渠道、最低金额、退款周期和额度换算规则。
4. 代理提成比例、计算基数、结算周期、退款冲正、税务、开票、提现和 KYC。
5. 用户须知正式正文、发布人、版本生效时间和是否需要再次同意。

已冻结、不得再次作为阻塞提出：企业用户和个人用户共用一个 Tenant 模型及统一创作工作台；直接注册自动创建单人 Tenant 和 `tenant_admin` Membership，后续通过补资料和加成员原地成长为多人企业 Tenant。
