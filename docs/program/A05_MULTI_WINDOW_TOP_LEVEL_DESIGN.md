...
> **一套个人与企业创作者共用的真实系统：用户可以用真实账号登录，在统一 Tenant 和创作工作台中用真实品牌与素材创建项目，在人工审批的关键节点下调用真实 AI 生成脚本、分镜、图片、视频和配音，最终导出可播放 MP4，并能解释每个任务、素材、回执和额度变化。**

用户真实黄金路径：

```text
白名单账号登录
→ 进入统一 Tenant（直接注册时为单人 Tenant）
→ 建立真实项目
→ 填写/辅助生成 Brief
→ 生成脚本并人工审批
→ 生成分镜并人工确认
