...
requirements and readiness. This GET is read-only and cannot repair missing
facts.
...
For first-day scope, every Package shot has exactly one deterministic
virtual-character requirement. Its initial document prompt equals the exact
approved storyboard description. Zero or multiple virtual-character assets do
not produce partial workspace facts; the endpoint returns the frozen 409 error.
...
The server validates base64 with bounded length and an iterative alphabet and
padding check before decoding and round-trip comparison. It must not apply a
single unbounded regular expression to the maximum payload. One canonical
decoded byte reaches the codec boundary but fails full materialization with
`CANVAS_MATERIALIZATION_MIME_UNSUPPORTED`; a three-byte JPEG magic prefix is
the minimum JPEG fixture. A valid JPEG at exactly 8 MiB passes, while 8 MiB + 1
fails `CANVAS_MATERIALIZATION_SOURCE_TOO_LARGE` without an uncaught exception.
...
The matrix contains 69 vectors covering strict workspace parity, canonical UTC
