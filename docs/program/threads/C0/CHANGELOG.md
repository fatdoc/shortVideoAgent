...
## 2026-08-10 · A-BIZ-06E A/B Golden Path Joint Gate Plan

- 新增 `A_BIZ_06E_A_B_GOLDEN_PATH_JOINT_GATE_PLAN.md`，冻结 06E.0～06E.6 原子顺序与 B baseline fail-closed 前置。
- 审计确认当前 B remote-tracking ref 仍是旧 D2 baseline；Pilot Script/Storyboard/Canvas 仍为 Demo Store/Grant 路径，A strict frontend client 尚无 Script/Storyboard/Production API。
...
## 2026-08-11 · A-BIZ-06E A-side Dual-Authority Canvas Chain Complete

- Migration 020—023 已覆盖 Storyboard authority、Canvas Entry lifecycle、Production Package Storyboard binding 与 Canvas exact Grant/Package/Project/Tenant binding。
- Production Package v0.3 strict HTTP 只接受当前 approved Script + approved Storyboard，保留真实 Cookie、no-store、Request ID 和安全 401/403/404/409/422/500。
- Grant issue/replay/introspection 与 Canvas create/replay/read/consume 统一复用 server-only Production Authority verifier；旧 v0.2、过期或 stale authority 均 fail closed。
- Canvas stale read 返回 410 且不写库；stale replay/consume 先持久化 `expired`，事务提交后再返回 410；consumed precedence 继续为 409 replayed。
- Browser Canvas DTO 保持 exact 9 keys；不暴露 raw Grant、token、grantId、digest、authority reason、Package snapshot、SQL 或 stack。
