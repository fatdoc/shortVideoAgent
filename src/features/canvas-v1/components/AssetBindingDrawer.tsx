...
export function AssetBindingDrawer({ asset, onClose, onBind }: AssetBindingDrawerProps) {
  return (
    <AnimatePresence>
      {asset ? (
        <motion.div className="cv1-drawer-layer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.aside className="cv1-drawer" role="dialog" aria-modal="true" aria-labelledby="cv1-drawer-title" initial={{ x: 30 }} animate={{ x: 0 }} exit={{ x: 30 }} transition={{ duration: .18 }}>
            ...
            <div className="cv1-drawer__preview">{asset.controlledPreviewUrl ? <img src={asset.controlledPreviewUrl} alt={`${asset.displayName}预览`} /> : <span>暂无受控预览</span>}</div>
            <dl>
            ...
            <button className="cv1-secondary-action" type="button" disabled={!asset.targetEntityId || asset.rightsStatus !== 'authorized' || asset.approvalStatus !== 'approved' || asset.providerStatus !== 'active'} onClick={() => onBind(asset)}>绑定到当前镜头</button>
