...
function runVendorCode(tsCode: string): Record<string, any> {
  const code = transform(tsCode, { transforms: ["typescript"] }).code.replace(/export\s*\{\s*\};?/g, "");
  const exports = {};
  new VM({ timeout: 1000, sandbox: { exports }, eval: false, wasm: false }).run(code);
  return exports;
}

function writeVendorCode(id: string, tsCode: string) {
...
function getVendor(id: string) {
  const source = fs.readFileSync(path.join(getPath("vendor"), `${id}.ts`), "utf8");
  return runVendorCode(source).vendor;
}
