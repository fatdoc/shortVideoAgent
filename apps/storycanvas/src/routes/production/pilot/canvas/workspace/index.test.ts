...
import { createCanvasV1WorkspaceRouter } from ".";
...
test("workspace GET returns only the strict hydration envelope", async (context) => {
  const app = express();
  app.use(installCanvasResponseBoundary);
  app.use(createCanvasV1WorkspaceRouter({
    resolveRequestScope: async () => scope,
    read: async () => fixture.workspaceResponse,
  }));
  ...
  const response = await fetch(`http://127.0.0.1:${address.port}/`, { headers: { "x-request-id": "req-workspace" } });
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), fixture.workspaceResponse);
  const serialized = JSON.stringify(await (await fetch(`http://127.0.0.1:${address.port}/`)).json());
  assert.equal(/contentBase64|storageReference|asset:\/\//u.test(serialized), false);
});
