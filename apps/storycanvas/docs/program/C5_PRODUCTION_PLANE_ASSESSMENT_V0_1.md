...
## 14. D1 Round 3 可播放 FALLBACK 增量说明

本节记录首轮只读评估之后、经 C0 明确授权完成的 D1 实现增量；与第 13 节“未修改产品源码”的首轮边界不冲突。

### 14.1 纯合成媒体

C5 使用本机 `ffmpeg 8.1.2` 和 lavfi `testsrc2 + sine` 生成：

`frontend/public/media/d1/demo-local-001-fallback-synthetic-v1.mp4`

媒体事实：

- 完全自生成，无下载素材、历史咖啡图、品牌照片或第三方音视频。
- 权利来源：`SELF_GENERATED_SYNTHETIC / NO_THIRD_PARTY_ASSET`
- 540×960、6 秒、30 fps、H.264/AAC、2,155,679 bytes
- SHA-256：`55370297920ad6f957a3bbcdb4cbdc2ff088ba7594062a07c589b7a6db3727ef`
- Truth：`FALLBACK`
- Delivery claim：`DEMO_ONLY`
- 明确非 FireRed、非真实 Provider、非 AI 成片、非正式海底捞营销素材。

### 14.2 生产模型更新

- shot-05 新增纯合成 FALLBACK repair asset。
- ExportArtifact 包含 8-shot/basic-merge 来源链、shot-05 repair 引用、真实文件 checksum、byte size、URL/path、duration、dimensions 和 codecs。
- `playable=true`，technical playback QA `passed`。
- editorial QA `not_evaluated`、brand QA `not_approved`；技术播放通过不得解释为正式业务 QA passed。
- `POST /projects/demo-local-001/fallback-export` 继续要求显式当前 grant，并产生 `receiptType=export` 的 Outbox receipt；领取、重投和 ack 沿用 pending/delivered/acknowledged 状态机。
- canonical UI 可直接播放并醒目标注“本地合成演示片，仅验证流程，不代表 AI 生成质量或正式交付”。
- legacy generation/export 门禁保持不变。

### 14.3 风险结论更新

“没有权利明确的可播放 Demo 媒体”已不再阻塞 D1 流程证明。仍未关闭且不得混淆的风险包括：

- 正式品牌素材与营销内容的 editorial/brand QA。
- FireRed 完整剪辑与真实 Provider 质量。
- Toonflow 商业授权、模型凭证与本地单用户架构风险。

本轮只执行获 C0 授权的 ffmpeg 生成与一次新媒体 ffprobe 元数据采集；未运行 test/build/lint/browser/governance，未提交或合并。

### 14.4 REQ-C7-014 ExportReceipt 对齐

FALLBACK Export Outbox payload 已补齐 C4 ExportReceipt Schema 要求的最小事实：

- `exportId=export-demo-local-001-fallback-v1`
- `exportArtifactId=exportId`（C5 扩展别名）
...
- `error=null`
- `truthMode=FALLBACK`
- tenant/project/package、`idempotencyKey`、`createdAt`

envelope `businessId` 与 `exportId` 一致，`payloadDigest` 继续由最终完整 payload 计算。FALLBACK、DEMO_ONLY、technical/editorial/brand QA、rights 和媒体元数据扩展全部保留；显式 grant 与 Outbox 三态不变。
