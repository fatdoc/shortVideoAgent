...
process.env.STORYCANVAS_ERROR_LOG_PATH ||= path.join(app.getPath("logs"), "storycanvas-error.log");
...
app.whenReady().then(async () => {
  try {
    ...
    // Vite renderer uses the stable localhost API so restarts keep the same
    // Electron user-data database and generated media. File builds continue
    // using a random port discovered through the toonflow:// protocol.
    const port = await mod.default(!process.env.VITE_DEV);
